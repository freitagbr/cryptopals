...
// because most of what we want to encrypt is better described as a stream than
// as a sequence of blocks. Daniel Bernstein once quipped to Phil Rogaway that
// good cryptosystems don't need the "decrypt" transforms. Constructions like
// CTR are what he was talking about.

std::string challenge_18(const std::string &cipher, const std::string &key) {
  return aes::ctr::decrypt(cipher, key);
}

} // namespace cryptopals

int main() {
  const std::string expected("Yo, VIP Let's kick it Ice, Ice, baby Ice, Ice, baby ");
  const std::string b64("L77na/nrFsKvynd6HzOoG7GHTLXsTVu9qvY/2syLXzhPweyyMTJULu/6/kXX0KSvoOLSFQ==");
  const std::string key("YELLOW SUBMARINE");

  try {
    const std::string cipher = cryptopals::base64::decode(b64);
    const std::string output = cryptopals::challenge_18(cipher, key);
